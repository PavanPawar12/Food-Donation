...
const MyDonations = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const { data } = await api.get('/donations/my-donations', { params: { page: 1, limit: 20 } });
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="text-gray-600">All donations you have posted</p>
        </div>

        {loading ? (
          <div className="text-center py-12">Loading...</div>
        ) : items.length === 0 ? (
          <div className="text-center py-12 text-gray-600">You haven’t donated yet. Start by posting your first donation!</div>
        ) : (
            ...
            {items.map((d) => (
              <div key={d._id} className="bg-white rounded-xl shadow p-6">
                <div className="flex justify-between items-start mb-3">
                  ...
                  <span className={`px-3 py-1 rounded-full text-xs font-medium ${d.status === 'available' ? 'bg-green-100 text-green-800' : d.status === 'claimed' ? 'bg-yellow-100 text-yellow-800' : 'bg-blue-100 text-blue-800'}`}>{d.status}</span>
                </div>
